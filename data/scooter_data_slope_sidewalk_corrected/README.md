...
Generated by `python -m experiments.scooter_correct` from `scooter_data_slope_sidewalk/`. Same rows and timestamps as the originals; every column is in the vehicle frame (x forward, y left, z up, angles positive left). See the script docstring for each correction.
